...
public class Exercise1 {
	...
	// SE PIDE: Proporcione una solución iterativa y otra recursiva final
	// equivalentes.
	...
	public static Map<Cuadrante, String> ejercicio1Iterativo(List<Punto2D> ls) {
		...
		while (i < ls.size()) {
			...
			if (punto.x() % 5 != 0) { // filtro
				...
				String textoX = nuevaX % 2 == 0 ? nuevaX + "¡" : nuevaX + "!"; // // Generar el texto de la coordenada x
																				// con "¡" si es par, "!" si es impar

				// Si el cuadrante ya tiene una cadena asociada, la concatenamos con "-"...
				// esto lo hacemos para que los valores del diccionario queden separados... -->
				// Cuadrante2: "2¡-10¡"
	...
	public static Map<Cuadrante, String> solucionIterativa(List<Punto2D> ls) {
		Integer e = 0;
		Map<Cuadrante, String> ac = new HashMap<>();
		while (e < ls.size()) {
			Punto2D punto = ls.get(e);
			if (punto.x() % 5 != 0) {
				double nuevaX = punto.x() + 3;
				Punto2D puntoTransformado = Punto2D.of(nuevaX, punto.y());
				Cuadrante cuadrante = puntoTransformado.cuadrante();
	...
	public static Map<Cuadrante, String> solucionRecursivaFinal(List<Punto2D> ls) {
		return recFinal(0, new HashMap<>(), ls);
	}

	private static Map<Cuadrante, String> recFinal(Integer e, Map<Cuadrante, String> ac, List<Punto2D> ls) {
		Map<Cuadrante, String> r = ac;
		if (!(e < ls.size())) {
			return ac;
		}
		Punto2D punto = ls.get(e);
